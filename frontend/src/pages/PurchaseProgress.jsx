...
const PurchaseProgress = () => {  
    ...
    const getCurrentStep = (order) => {
        ...
        } else { // BANK_TRANSFER hoặc phương thức thanh toán khác
            switch (status) {
                case "PENDING": return 0;
                case "PAID": return 1;
            ...
            <div className="order-details">
                ...
                <div className="order-info">
                    ...
                    <div className="info-row">
                        ...
                        <span className="total-price">{new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(order.totalPrice)}</span>
                    </div>
                </div>
            </div>
        );
                    ...
                    {order.orderItems.map(item => (
                        <div key={item.id} className="item">
                            ...
                            <div className="item-details">
                                <div className="item-name">{item?.variantResponse?.productResponse?.productName}</div>
                                <div className="item-variant">Phân loại: {item?.variantResponse?.variantName || "Mặc định"}</div>
                                <div className="item-quantity">Số lượng: x{item.quantity}</div>
                            </div>
                            <div className="item-price">
                                <div className="price">{new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(item?.variantResponse?.discountPrice || item?.variantResponse?.price)}</div>
                            </div>
                        </div>
                    ))}
                </div>
    ...
    if (loading) {
        return <div className="loading-container">Đang tải thông tin đơn hàng...</div>;
    }
    ...
    const status = currentStep === -1 ? "error" : currentStep === -2 ? "finish" : "process";
            ...
            <div className="progress-steps">
                {currentStep === -1 ? (
                    <div className="canceled-order">
                        ...
                        <div className="canceled-date">Ngày hủy: {new Date(order.updatedAt).toLocaleDateString('vi-VN')}</div>
                    </div>
                ) : currentStep === -2 ? (
                    <div className="refunded-order">
                        <div className="refunded-icon">↩️</div>
                        <div className="refunded-text">Đơn hàng đã được hoàn tiền</div>
                        <div className="refunded-date">Ngày hoàn tiền: {new Date(order.updatedAt).toLocaleDateString('vi-VN')}</div>
                    </div>
                ) : (
                    <Steps 
                        current={currentStep}
