...
const AdminCategory = () => {
  ...
  const handleCreateCategory = async () => {
    ...
    if(imageCreateRequest){
        ...
        formdata.append('category', new Blob([JSON.stringify(formCreateCategory)], { type: "application/json" }));
  ...
  const handleEditCategory = async () => {
    ...
    if(imageUpdateRequest){
        const formdata = new FormData();
        formdata.append('category', new Blob([JSON.stringify(formUpdateCategory)], { type: "application/json" }));
  ...
  const totalProducts = categories.reduce((sum, cat) => sum + cat.totalProduct, 0);
